...
func GetRankingBaseSale(c echo.Context) error {
	...
	var staffID []string
	for k, _ := range listStaffId {
		staffID = append(staffID, k)
	}

	if err := dbSale.Ctx().Raw(sql, quarter, quarter, quarterNum, quarterNum, staffID).Scan(&report).Error; err != nil {
	...
	if err := dbSale.Ctx().Raw(sqlBefore, yearBefore, quarterBefore, quarterBeforeNum, yearBefore, staffID).Scan(&invBefore).Error; err != nil {
		if gorm.IsRecordNotFoundError(err) {
			return c.JSON(http.StatusNoContent, nil)
		}
		log.Errorln(pkgName, err, "select data error :-")
